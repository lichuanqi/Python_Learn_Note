...
def main():
    ...
    parser.add_argument(
        "--test-file",
        default="examples/zh_normalization/data/data_test.txt",
        type=str,
    ...
    wf_raw = open(raw_path, "w", encoding='utf-8')
    wf_ref = open(ref_path, "w", encoding='utf-8')

    with open(test_file, "r", encoding='utf-8') as rf:
        for i, line in enumerate(rf):
            raw_text, normed_text = line.strip().split("|")
            wf_raw.write("utt_" + str(i) + " " + raw_text + "\n")
            wf_ref.write("utt_" + str(i) + " " + normed_text + "\n")

    print('data propress over')
