...
if __name__ == "__main__":
    ...
    plt.plot(epoch,val_acc_smooth,label='train_acc')
    # 坐标轴
    plt.xlabel('Epoch')
    plt.ylabel('Acc')
    # 坐标轴刻度字体
    plt.minorticks_on()
    plt.legend()
    # plt.savefig('D:/论文/论文3-毕业论文/图片/图 20-1-准确率',dpi=300, bbox_inches='tight')

    plt.figure()
    plt.plot(epoch,loss_smooth,label='train_loss')
    plt.plot(epoch,val_loss_smooth,label='train_loss')
    # 坐标轴
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    # 坐标轴刻度字体
    plt.minorticks_on()
    plt.legend()
    # plt.savefig('D:/论文/论文3-毕业论文/图片/图 20-2-损失函数',dpi=300, bbox_inches='tight')
